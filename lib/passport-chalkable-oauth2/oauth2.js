...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get('https://classroom.chalkable.com/Person/Me.json', accessToken, function (err, body, res) {
    ...
    try {
      ...
      profile.displayName = json.data.fullname;
      profile.name = {
        familyName: json.data.lastname,
        givenName: json.data.firstname
      };
      profile.role = json.data.role;
      if (json.data.email)
        profile.emails = [{ value: json.data.email }];
      if (json.data.pictureid)
        profile.photos = [{ value:'https://classroom.chalkable.com/PersonPicture/Read?pictureId=' + json.pictureid + '&width=' + json.pictureheight + '&height=' + json.picturewidth }];
